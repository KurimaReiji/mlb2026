...
const find_team = (team) => (db) => {
  return db.find((o) => o.nickname.toUpperCase() === team.toUpperCase());
}

const url = `./mlb2026-results.json`;
...
const head2head = (team) => (games) => {
  ...
  const opponents = myGames.map((game) => opponent(team.toUpperCase())(game)).reduce(to_uniq);
